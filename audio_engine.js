...
(function(){
  ...
  /* ── withCtx: schedule notes AFTER context is confirmed running ── */
  function withCtx(fn){
    if(muted) return;
    const c = getCtx();
    if(c.state === 'running'){
  ...
  /* ── helpers ── */
  function note(freq, start, dur, vol=0.3, type='sine', c){
  ...
  function noise(c, start, dur, vol=0.15){
  ...
  window.playLevelUp = function(){
    withCtx(c => {
      ...
      melody.forEach((freq, i) => note(freq, now + i*0.09, 0.22, 0.22, 'triangle', c));
